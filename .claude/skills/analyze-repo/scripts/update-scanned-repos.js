...
const [, , scannedFile, url, hooksFound, hooksAdded, status = 'success'] = process.argv
if (!scannedFile || !url) {
  console.error('usage: update-scanned-repos.js <scanned-repos.json> <url> <hooks_found> <hooks_added> [status]')
  process.exit(1)
}
...
if (idx >= 0) {
  repos[idx] = entry
  console.log(`Mise à jour : ${name} (${entry.hooks_found} hooks trouvés, ${entry.hooks_added} ajoutés)`)
} else {
  repos.push(entry)
